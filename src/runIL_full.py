...
def class_aware_representation(model, buffer, data_loader, task_id):
    x_list = []
    mask_list = []
    y_list = []
    fea_list = []
    ...
    if clus == 'hearding':
        ...
        running_times['hearding'].append(time.time() - start_time)  # Record the time

    elif clus == 'rainbow':
        start_time = time.time()  # Start timing
        # Rainbow-based selection to maintain exemplar diversity
        mean_fea = np.mean(fea_list, axis=0)
        distances = [(i, np.linalg.norm(mean_fea - fea_list[i])) for i in range(len(fea_list))]
        distances = sorted(distances, key=lambda x: x[1])

        selected_indices = [i for i, _ in distances[:int(store_ratio * len(x_list))]]
        for i in selected_indices:
            buffer.append((x_list[i], mask_list[i], y_list[i], task_id))

        running_times['rainbow'].append(time.time() - start_time)  # Record the time
        ...
        running_times['gmm'].append(time.time() - start_time)  # Record the time
        ...
        running_times['random'].append(time.time() - start_time)  # Record the time
    print("Buffer size:{}".format(len(buffer)))
    print(buffer.labels)
    b_lbl = np.unique(buffer.labels)
    for i in b_lbl:
        print("Label {} in Buffer: {}".format(i, buffer.labels.count(i)))
    avg_running_times = {key: np.mean(values) for key, values in running_times.items() if values}

    with open('running_times_log.txt', 'w') as f:
        for algorithm, avg_time in avg_running_times.items():
            f.write(f"{algorithm}: {avg_time:.4f} seconds\n")
...
def train_step(model, optimizer, distill_CR, cls_CR, x, mask, y, t, task_id, replay,
    ...
    loss = lam*cls_loss + (1-lam)*distill_loss
...
def runRnD():
    ...
    for task_id in range(task_num):
        ...
        class_aware_representation(model, buffer, data_loader, task_id)
